...
def envSetup(newFreq_in, domainSize, minim=400, maxim=600):# domain size and freq 
   # P.epsRe =1
    ...
    Nlam =400#np.floor(20*np.sqrt(P.epsRe*P.muRe))
    dz =lamMin/Nlam
    #P.courantNo = 1   # LOOK INTO 2D VERSION
    delT = 0.1*(dz/c0)#0.95/(P.c0*np.sqrt(1/(P.dz**2)))
   # decimalPlaces =11
   # multiplier = 10 ** decimalPlaces
    ...
    if courantNo > 1.01 or courantNo <0:
        ...
        sys.exit()
    pmlWidth = 3.5*int(lamMin/dz) #+int((30*P.freq_in)/1e9)
