...
forLocals = {'v0': float64[:,:], 'vt' : float64[:], 'H': float64[:,:]}
...
def luInvert(a, name = "Null"):
   ...
   # M = sparse.linalg.spilu(a.todense())
    Splat = sparse.linalg.spilu(a)
       ...
       # breakpoint()
        ...
        if (pulseInit == False) or (pulseInit == True and jj >= delayMOR):
           ...
           # print("shapes of y1, summer_red: ", y1.shape, summer_red.shape)
            ...
            XnP1, info = sparse.linalg.lgmres(summer, y, x0=XnP1.todense(), atol=1e-4, M = MM1, maxiter = 50)
            if info != 0:
                print("iterations: ", info)
            checkNan(XnP1, eval(checkLine), jj)
