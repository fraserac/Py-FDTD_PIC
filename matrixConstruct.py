...
def ABuild(P, De, Dh, K, Kt, blocks=2):  # VERY SPARSE UPDATE MATRIX OVER ALL SPACE
...
def solnDenecker(R, F, UnP1A, UnP1B, Xn, V, P, C_V, C_P):
    Exs, Hys = BF.SmoothTurnOn(V,P)
    print("MOR Time stepping...")
    ...
    for jj in range(0,P.timeSteps):
       ...
       # y = y.reshape(len(y),1)
        ...
        XnP1 = sparse.linalg.spsolve((R+F), y -B@UnP1)  # outputs NONSPARSE
        XnP1 = XnP1.reshape(len(XnP1), 1)
        #XnP1 = sparse.csc_matrix(XnP1)
