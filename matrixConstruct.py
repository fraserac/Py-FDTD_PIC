...
def ExampleArnoldi(A, v0, k): # Issue with [0,0] 
    ...
    H = np.ones((k+1,k))*(0+0j) # ANNOYING WAY TO MAKE COMPLEX MATRIX?!
   ...
   # print(numba.typeof(v0))
    ...
    split =  np.floor(k/10)  
    count =1    
    vt = np.ones(len(v0))*(0+0j)
    vt = np.ascontiguousarray(vt)
   # breakpoint()
    for m in range(k):
        if m%50 ==0:
            print(" this far through arnoldi: ", m, "/", k, "split : ", split)
        vt = A@v0[ :, m]
        vt = np.ascontiguousarray(vt)
...
def luInvert(a, name = "Null"):
    ...
    M = splu(a)
    print("SPLU completed")
    M = M.solve(In)
    
    print("solved LU In")
    M = sparse.csc_matrix(M)
    check = np.allclose(a@M.todense(), np.identity(length), atol = 1e-4)
   # breakpoint()
    if check == False:
        print("not the inverse!", check)
        print((a@M.todense())[0,0])
       #sys.exit() 
...
def solnDenecker(R, F, A, UnP1A, UnP1B, Xn, V, P, C_V, C_P, pulseInit = True):
    delayMOR = P.delayMOR
    Exs, Hys = BF.SmoothTurnOn(V,P)
   ...
   # breakpoint()
    ...
    v0, R = np.linalg.qr(v0)   # if matix is complex returns unitary, normalise it...
   ...
   # V_sp*= modify
    ...
    #V_sp = np.block([[V_sp[:int(V_sp.shape[0]/2), :], np.zeros(int(len(V_sp)/2), k)] , [np.zeros((int(len(V_sp)/2), k)), V_sp[V_sp.shape[0]/2:,:]]])
    # ABOVE LINE IS UNREADABLE, SPLIT INTO MULTILINE
   ...
   # breakpoint()
    ...
    summer_red_inv = sparse.csc_matrix(summer_red_inv)
    summer_red_sparse = sparse.csc_matrix(summer_red)
   ...
   # summer_red = sparse.csc_matrix(summer_red)
    ...
    sos = sig.butter(21, 0.02, output='sos')
    ...
    for jj in range(0,P.timeSteps):
       ...
       # y = y.reshape(len(y),1)
        ...
        UnP1_red = V_spt@UnP1#Q_spt@UnP1
       ...
       # UnP1_red = sparse.csr_matrix(UnP1_red)
        ...
        if (pulseInit == False) or (pulseInit == True and jj >= delayMOR):
            ...
            XnP1_red = XnP1_red.reshape(len(XnP1_red), 1)
            y1 = winter_red@XnP1_red 
            y1 +=B_red@UnP1_red
           # print("y shape if first before solve: " ,y.shape)
            #XnP1 = A@XnP1 + B@UnP1
            #breakpoint()
            y1 = sparse.csc_matrix(y1)
           # breakpoint()
           # XnP1_red = summer_red_inv@y1
            XnP1_red = sparse.linalg.spsolve(summer_red, y1)
