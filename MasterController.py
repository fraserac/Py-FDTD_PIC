...
class Variables(object):
    ...
    def __init__(self, Nz, timeSteps):
        ...
        self.Hy_History = self.Ex_History
        self.Jx_History = self.Ex_History
        self.polCurr_History = self.Ex_History
        self.Dx_History = self.Ex_History
...
class CPML_Params(object):
    def __init__(self, dz):
        ...
        self.r_scale = 5# Within ideal bounds see Journal of ELECTRICAL ENGINEERING, VOL 68 (2017), NO1, 47–53, see paragraph under eqn. 17 (scaling power is called 'm' )
        self.r_a_scale=1.0
        self.sigmaEMax=1.1*(0.8*(self.r_scale)/(dz*(sci.constants.mu_0/sci.constants.epsilon_0)**0.5))#1.1*sigmaOpt # Within ideal bounds for value, : Journal of ELECTRICAL ENGINEERING, VOL 68 (2017), NO1, 47–53, see paragraph under eqn. 17
...
def Controller(V, P, C_V, C_P,Exs, Hys):
    ...
    for i in range(0, 2):
        ...
        for counts in range(0,P.timeSteps):
               ...
               V.Hy =BaseFDTD11.CPML_HyUpdate(V, P, C_V, C_P)
               ...
               #if i == 1: 
                 #   V.Dx, V.Ex = BaseFDTD11.ADE_DxUpdate(V, P, C_V, C_P)
                    #print(V.Dx, "Dx master")
                    #V.Dx[P.nzsrc] = BaseFDTD11.ExTfSfCorr(V,P, counts, Hys)
...
setupReturn=envDef.envSetup(freq_in, domainSize, 600, 700)
