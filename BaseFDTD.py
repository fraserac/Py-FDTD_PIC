...
def CPML_FieldInit(V,P, C_V, C_P):#INITIALISE FIELD PARAMS 
    C_V.kappa_Ex =np.zeros(P.Nz)
    C_V.kappa_Hy = np.zeros(P.Nz)
...
def CPML_ScalingCalc(V, P, C_V, C_P):
    ...
    for nz in range(P.Nz):   #SCALES VARS AWAY FROM SIM DOMAIN, REVERSE FOR LOOP
        C_V.kappa_Ex[nz] =1+(C_P.kappaMax-1)*((P.pmlWidth-nz)/(P.pmlWidth-1))**C_P.r_scale
        
        C_V.sigma_Ex[nz] = C_P.sigmaOpt*((P.pmlWidth - nz)/(P.pmlWidth-1))**C_P.r_scale   #np.abs((((np.abs(nz -0.75))/P.pmlWidth)**(C_P.r_scale))*C_P.sigmaEMax)
        
        C_V.alpha_Ex[nz] = C_P.alphaMax*(nz-1/P.pmlWidth-1)**C_P.r_a_scale #np.abs((1-((counter)/P.pmlWidth)**C_P.r_a_scale)*C_P.alphaMax)
        
        
     for nz in range(P.Nz-1):   
        C_V.kappa_Hy[nz] =1+(C_P.kappaMax-1)*((P.pmlWidth-nz-0.5)/(P.pmlWidth-1))**C_P.r_scale
        C_V.sigma_Hy[nz] = C_P.sigmaOpt*((P.pmlWidth -nz-0.5)/(P.pmlWidth-1))**C_P.r_scale   #np.abs(((C_P.sigmaEMax)*(np.abs(nz -0.75)/P.pmlWidth)**(C_P.r_scale))*C_P.sigmaHMax)
        C_V.alpha_Hy[nz] = C_P.alphaMax*(nz-0.5/P.pmlWidth-1)**C_P.r_a_scale#np.abs(((1)*(1-(counter)/P.pmlWidth)**(C_P.r_a_scale))*C_P.alphaMax)
...
def CPML_Ex_Update_Coef(V,P, C_V, C_P):
    for nz in it.chain(range(0, P.pmlWidth), range(P.Nz-P.pmlWidth-1, P.Nz)):
        C_V.eLoss_CPML[nz] = (C_V.sigma_Ex[nz]*P.delT)/(2*P.permit_0)
        
        C_V.Ca[nz] = (1-C_V.eLoss_CPML[nz])/(1+C_V.eLoss_CPML[nz])
        C_V.Cb[nz] = P.delT/((1+C_V.eLoss_CPML[nz])*P.permit_0*C_V.kappa_Ex[nz]*P.dz)
...
def CPML_Hy_Update_Coef(V,P, C_V, C_P):
    for nz in it.chain(range(0, P.pmlWidth), range(P.Nz-P.pmlWidth-1, P.Nz)):
        ...
        C_V.C1[nz] =1 #(1-C_V.mLoss_CPML[nz])/(1+C_V.mLoss_CPML[nz])
        C_V.C2[nz] = P.delT/P.permea_0#P.delT/((1+C_V.mLoss_CPML[nz])*P.permea_0*C_V.kappa_Hy[nz]*P.dz)
...
def denominators(V, P, C_V, C_P):
    ...
    for j =1:P.Nz 
        if j <= P.pmlWidth -1:
            den_Hydz[j]= 1/(C_V.kappa_Hy[j]*P.dz)
        else if j>= P.Nz+1-P.pmlWidth:
            den_Hydz[j] = 1/(C_V.kappa_Hy[jj]*P.dz)
            jj-=1
        else:
            den_Hydz[j] = 1/(P.dz)
    jj = P.pmlWidth
    for j =1:P.Nz 
        if j <= P.pmlWidth:
            den_Exdz[j]= 1/(C_V.kappa_Ex[j]*P.dz)
        else if j>= P.Nz+1-P.pmlWidth:
            den_Exdz[j] = 1/(C_V.kappa_Ex[jj]*P.dz)
            jj-=1
        else:
            den_Exdz[j] = 1/(P.dz)        
...
def CPML_ExUpdate(V,P, C_V, C_P):
    for nz in range( 1, P.Nz):
        V.Ex[nz]= C_V.Ca[nz]*V.Ex[nz]+C_V.Cb[nz]
    return V.Ex
